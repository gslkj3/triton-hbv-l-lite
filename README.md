...
# L-lite loop autotune control

This public branch is a stripped, standalone research control built on
Triton 3.6.0.  It enumerates the Cartesian product of an independent Loop
Bridge intervention and one of three loop routes, materializes each admissible
candidate, and delegates winner selection to Triton's native exhaustive
autotuner.
...
IR facts
  -> Bridge factor
  -> route subject and route-factor legality
  -> Bridge-then-route materialization
  -> final artifact/correctness qualification
  -> native Triton autotune over the complete requested domain
  -> native cache
```

The three routes are software pipeline, full unroll plus phase-major reorder,
and full unroll plus logical-group vectorization.  Dynamic exact-prefix
materialization is represented as a subtype of the vectorization route, not as
a fourth optimization mechanism.  Bridge factor and route factor remain
independent parameters; equality is imposed only where the current physical
materializer requires it.

This repository contains only the exhaustive-autotune comparison arm and its
shared legality/materialization surface.  Historical development records,
private experiment logs and machine-local measurements are not part of the
publication.  Performance measurements remain frozen until the capability
surface is closed and a clean measurement environment is available.
...
- `lib/Dialect/Triton/Transforms/LoopUnroll.cpp`: loop-unroll support shared by
  the routes;
- `test/Triton/hbv-loop-bridge-*.mlir`: compiler-level capability checks;

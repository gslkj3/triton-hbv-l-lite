...
> 2026-09-17 开发补充：[循环向量化的信息传播补全：Join/Split 修复记录](join-split-analysis-repair.zh-CN.md)。
> 说明原生分析覆盖缺口、必要性与粒度审查、受控测试结果及创新声明边界。
> 此次同步的是报告，不代表公仓编译器及历史全库收益已更新。
...
>
> L-lite 实现快照：`7dfcfb898a534cf0af2cf2d63741e9f77e8ec414`
>
> 本次修订：补充 43 个重点算子的定向测试、逐 Route 候选缺口审计，并同步测试实际使用的
> 规则式物化快照。
...
pipeline。这些源码测试本身也不能替代 GPU 性能测量；下面单列的性能结果来自独立测量
campaign（测量批次），不扩大这里的源码测试覆盖范围。

### 16.8 三项 PR 的冻结测试结果

这里的 PR（合并请求）测试回答两个不同问题：

1. 编译器能否自动识别并物化手工 PR 所表达的核心循环变换；
2. 自动生成的代码放回公开调用路径后，能否完整复现手工 PR 的端到端收益。

这两个问题不能混为一谈。前者验证 Bridge 和 Route 的自动编译能力，后者还会受到公开
封装、shape（形状）选择和 tile（分块）策略影响。冻结结果如下。

| 测试对象 | 样例数 | 自动优化覆盖 | 自动直接内核几何平均加速 | 手工 PR 几何平均加速 | 自动核心变换与手工对应版本的关系 | 自动调优耗时 | 首次获得结果的总耗时 |
|---|---:|---:|---:|---:|---:|---:|---:|
| PR2955 | 9 | 9/9 | 1.129× | 1.117× | 对应 Route 的中位时间比几何平均为 1.006，区间重叠 9/9 | 139.36 秒 | 141.14 秒 |
| PR4125：pack（打包） | 17 | 13/17 | 1.587× | 1.548× | 自动连续前缀版本与手工前缀版本的中位时间比为 0.992 | 123.70 秒 | 127.39 秒 |
| PR4126：unpack（解包） | 15 | 12/15 | 1.807× | 1.536× | 自动连续前缀版本与手工前缀版本的中位时间比为 0.992 | 12.09 秒 | 14.59 秒 |
| PR4125 + PR4126 合计 | 32 | 25/32 | 1.686× | 1.542× | 正确物化 32/32，计时区间重叠 29/32 | 135.79 秒 | 141.98 秒 |

PR2955 的 9 个样例全部找到了自动优化候选。最终获胜者中，8 个使用完全展开加重排，1 个
使用软件流水。自动生成的、与手工 factor（因子）和 Route 完全对应的候选，其计时与手工
版本基本重合；这说明这里复现的不是“碰巧选到另一段更快代码”，而是编译器确实物化了
手工 PR 所使用的核心变换。自动获胜者的加速范围为 1.098× 至 1.187×。

PR4125 和 PR4126 的核心机制是连续前缀向量化，它仍属于“完全展开加向量化”的规则式
子情形，并不是第四种 Route。直接调用变换后的内核时，自动版本已经复现了手工前缀变换；
但经 public wrapper（公开调用封装）运行时，32 个样例的几何平均加速只有 0.555×，即整体
发生退化。原因是手工 PR 还包含按 shape 选择 tile 的策略，而当前 L-lite 只自动复现了循环
变换，没有一并复制该外围策略。因此这里可以证明“核心编译变换已自动复现”，不能宣称
“公开接口的完整收益已经复现”。这项负结果保留在报告中，防止用直接内核数据替代真实
调用路径。

公平性方面，三个 PR 批次都没有在自动调优前替它删除笛卡尔积候选。PR2955 共提交 504 个
候选，自动调优后有 207 个被类型化为无效；PR4125 和 PR4126 合计提交 603 个，自动调优后
有 418 个无效。无效候选仍然产生了编译或尝试开销，因此上表的自动调优时间包含这部分
代价。

### 16.9 FlagGems（GPU 算子库）与 FlagGems-vLLM 全量自然算子结果

本节给出冻结全量测试的最终结果，替代此前仅覆盖 18 个 FlagGems 节点的阶段性快照。测试
对象不是挑选过的“容易成功”样例，而是冻结 inventory（测试清单）中的 279 个真实
pytest（Python 测试框架）算子实例：FlagGems 239 项，FlagGems-vLLM 40 项。每一项都必须
有 L-lite 终态、Original（原始编译路径）对照终态和类型化根因；不能只展示有收益的算子。

这批测量允许 GPU（图形处理器）上存在共享负载，因此定位为开发级结果。它可以回答“代码
是否正确”“候选是否真正物化”“自然算子中是否存在收益机会”“穷举 autotune（自动实测
选优）的实际代价有多大”，但不能替代论文最终使用的独占、高精度性能证书。特别是微秒级
kernel（内核）的极端点值需要在独占环境中重新确认。

完整材料分成两份：

- [`l-lite-full-library-public-summary-r1.md`](l-lite-full-library-public-summary-r1.md) 是面向评审的
  全量摘要，列出分仓库收益、所有正收益项、所有候选获取超时项和根因分布；
- [`l-lite-full-library-per-item-attribution-r1.md`](l-lite-full-library-per-item-attribution-r1.md)
  是适合网页直接阅读的 279 项逐算子归因表，保留每项终态、成对根因、请求数量、收益与
  获取耗时；
- [`l-lite-full-library-paired-item-root-causes-r1.md`](l-lite-full-library-paired-item-root-causes-r1.md)
  是 279 项逐算子、逐 kernel、逐循环、逐候选拒绝的完整账本。文件较大，是审计证据，不建议
  从头顺序阅读；评审时可按算子名或根因检索。

V47（第 47 版全量成对证据）的 SHA256（内容哈希）为
`900fce65cda334c59cec009f5780072d2efc429ea4eb0f9eef2c87b1ee612049`；V48（第 48 版完整性
审计）的 SHA256 为
`060b425ca4494c35a69bb14f30f47aeba6b5b233dfea73abc2233d4a5f5ba625`。V48 的 11 项检查全部
通过，证明 279 项身份无缺失、3396 个请求与 kernel 账本一一对应、收益只来自同一请求中的
直接计时，并且每个 L-lite 特有超时都有 Original 对照和编译进度证据。

#### 全量结果总览

| 仓库 | 算子项 | 测试通过项 | 至少一次选择非 Original 的项 | 收益合格请求/全部已观测请求 | 非 Original 获胜请求 | 全部合格请求几何平均 | 获胜请求几何平均 | 已闭合候选获取耗时 | L-lite 特有获取超时 | 实际提交候选 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| FlagGems | 239 | 170 | 83 | 3144/3202 | 419 | 1.0127× | 1.0996× | 13483.39 秒 | 39 | 59027 |
| FlagGems-vLLM | 40 | 20 | 5 | 160/194 | 40 | 1.0173× | 1.0711× | 1864.58 秒 | 6 | 3827 |
| 合计 | 279 | 190 | 88 | 3304/3396 | 459 | 1.0130× | 1.0971× | 15347.97 秒 | 45 | 62854 |

“至少一次选择非 Original”不等于“整个算子已经证明盈利”。88 项中有 75 项的非 Original
获胜请求几何平均大于 1×；其余项包含与 Original 持平或因共享环境噪声发生微小反转的
选择。上表的 1.0130× 是把收益合格请求中仍选择 Original 的请求按 1× 一起纳入后的结果，
比只报告获胜请求的 1.0971× 更接近实际全体请求效果。

收益合格还要求所在 pytest 项最终通过且角色原子闭合。失败、跳过、非原子文件证据和超时
中的局部计时均不进入收益聚合。45 个 L-lite 特有超时之外，还有 1 个双方无法形成确定
acquisition（候选获取）比较的超时项，因此 V48 的总超时项数是 46。

#### 有代表性的自然算子收益

下面不是重新挑选一套测试集，而是从全量账本中提取机制清楚、覆盖较宽或产品意义较强的
结果。完整 75 项正收益列表仍以公开摘要为准。

| 算子 | 请求覆盖 | 主要获胜机制 | 获胜请求几何平均 | 候选获取耗时 | 如何理解 |
|---|---:|---|---:|---:|---|
| `test_ctc_loss` | 8/8 个请求选择优化 | 完全展开加向量化的逻辑分组，factor 4/8 | 1.5333× | 103.90 秒 | 覆盖完整且收益大，是较强的自然算子正例 |
| `test_cp_gather_indexer_k_quant_cache_benchmark`（FlagGems-vLLM） | 4/4 | Bridge 4/8 + 完全展开加重排 factor 4/8 | 1.3398× | 111.10 秒 | 证明 Bridge 与 Route 联合物化能够在真实量化缓存算子上获益 |
| `test_softmax_backward` | 59/75 | 软件流水 factor 2/3/4 | 1.2428× | 184.96 秒 | 多 shape（形状）上形成较宽覆盖，不是单点命中 |
| `test_broadcast_to` | 4/6 | 完全展开加重排 factor 16 | 1.1827× | 153.92 秒 | 重排机制在自然广播算子上稳定参与竞争 |
| 四个 `scaled_mm_out` FP8 参数项 | 每项 3–4/12 | 软件流水 factor 3/4、完全展开加重排 factor 2 | 1.0829×–1.1440× | 每项 57.42–61.17 秒 | 稳态有小幅收益，但获取成本高，需热点复用才能摊销 |
| `mhc_pre`（FlagGems / FlagGems-vLLM） | 12/40、13/40 | 软件流水 + 完全展开加重排 | 1.0326×、1.0348× | 777.27、724.58 秒 | 两个仓库候选结构一致，收益方向复现，但全量 autotune 很昂贵 |

`test_tril_out_sliced` 有一个请求报告 9.0547×，但该项只有 1 个非 Original 获胜请求，而且
属于共享环境中的微秒级测量。它保留在完整结果中，不能删除；同时也不能在独占重测前把它
当作稳定的产品收益或论文结论。这样的处理比仅按点值排序更符合工程评审和论文审计要求。

#### 没有收益同样是重要结果

`mhc_bwd` 在 FlagGems 与 FlagGems-vLLM 中都完整执行了 6 个请求。两边均提交 314 个候选、
产生 255 个调优后无效候选，最终 6/6 都由 Original 获胜。候选获取耗时分别为 622.55 秒和
601.27 秒。候选结构和选择结果跨仓库完全一致，说明这不是偶发噪声，而是“花费大量
autotune 成本后仍无稳态收益”的可复核负例。

类似地，`test_mm` 和 `test_mm_out` 各执行 160 个请求、提交 1920 个候选、产生 1760 个
调优后无效候选，候选获取耗时约 199–200 秒，最终没有可执行非 Original 候选。L-lite 不在
提交前替 autotune 剔除这些组合，因此这些成本真实体现了穷举对照组的劣势。

这类结果正是主 L 预测体系存在的理由：L-lite 证明候选域和物化能力，并用完整实测给出上限
对照；主 L 则应只对极热、长期复用且预测收益下界足够的候选进行发布，避免在一次性或低
复用 kernel 上支付数百秒 acquisition。

#### 超时与失败如何归因

全量账本中有 45 个 `L_LITE_SPECIFIC_CANDIDATE_ACQUISITION_TIMEOUT`（L-lite 特有候选获取
超时）项。它们都满足同一证据条件：Original 在 900 秒内终止；L-lite 在候选已经提交后达到
900 秒边界；冷缓存中的 TTIR、TTGIR、LLVM IR（中间表示）、PTX（GPU 汇编表示）或 cubin
持续增长。因而这些项是 autotune 获取成本证据，不是死循环，也不产生收益标签。

典型模式并不完全相同：

- `pixel_shuffle_out`、`segment_reduce` 等属于大量普通候选累积，超时前分别生成约 900 或
  1000 量级的 cubin；
- `nanmedian_dim_values` 和 `hc_split_sinkhorn_forward` 属于少量但非常复杂的候选，单次
  TTIR 到后端产物的转换可持续数分钟；
- 卷积、池化和输入准备路径还会把辅助 Triton kernel 纳入完整候选枚举，因此超时成本应归到
  “全调用路径的穷举”，不能全部算到目标算子主体上。

其他未成功项不会统一写成“L-lite 不支持”。V48 的成对根因还包括：71 项没有可执行的非
Original 候选、11 项最终调用路径没有进入 Triton JIT（即时编译）、9 项到达最终 JIT 但没有
形成候选请求、21 项因冻结硬件或依赖条件跳过，以及少量双方共同依赖失败、硬件能力不匹配、
Provider（事实提供器）字段缺口或 L-lite 特有运行失败。每一项的具体错误文本、候选拒绝类和
Original 对照都保存在完整 V47 报告中。

#### 本轮测试得到的架构结论

第一，Bridge、软件流水、完全展开加重排和完全展开加向量化已经能经由编译器入口在真实
自然算子上自动参与竞争，并非只存在于三个手工 PR（合并请求）复现样例中。

第二，完整笛卡尔积 autotune 确实能够找到较大收益，但代价非常高：已闭合请求累计候选获取
约 15348 秒，另有 45 个 L-lite 特有角色各触及 900 秒边界。L-lite 因而适合作为能力基线和
实验对照，不适合作为所有生产 kernel 的默认运行策略。

第三，主 L 与 L-lite 必须继续共享 CandidateCompiler（候选编译器）、合法性、物化和正确性
证据；主 L 的核心价值不是生成另一套候选，而是用论文可信的因果模型、热点门和收益下界，
以远少于全量 autotune 的获取开销选择接近最优的候选。

### 16.10 43 个重点算子的定向补充测试

全量自然算子测试之后，又按评审指定清单对 43 个重点算子进行了定向测试。该批次不是只挑
有收益的算子：76 个原始精确角色全部获得终态；其中旧证据里只有聚合拒绝计数、无法回答
“哪条 Route 在哪一步断开”的 17 个角色，又用冻结编译器原样重跑，补齐到逐 Route 和逐候选
拒绝账本。`swiglu` 和 `gemma_rms_norm_kernel` 的原测试入口没有形成真实 L-lite 请求，因此另用
独立 PyTorch 参考值补测；补测只修正测试入口，不修改编译器和候选规则。

完整报告见
[`l-lite-priority-operators-43-report.zh-CN.md`](l-lite-priority-operators-43-report.zh-CN.md)。
本节只保留架构层结论。

第一，1057 个可计时请求中有 328 个选择非 Original，共 13 个算子找到收益。覆盖了 Bridge
加重排、Bridge 加软件流水、直接软件流水、直接重排和直接向量化，不是单一机制或单一仓库的
结果。其中 `cp_gather_indexer_k_quant_cache` 为 1.3398×，`apply_rotary_pos_emb` 为 1.1685×，
`grouped_topk` 在 324 个请求中有 135 个选择优化，整体为 1.0509×。

第二，Bridge 自身没有出现“具体 factor 已证明合法，但 Bridge 候选没有形成”的缺陷。静态
允许考虑 Bridge、实际却没候选的请求，均是当前 grid 上不存在 2/4/8/16 的合法完整分组，
属于正常的 factor 不存在。

第三，Route 侧确认了三类共享 CandidateCompiler 契约缺陷：候选参数与最终 Loop V2 准入
字段不一致；规划时的循环 subject 到最终物化时身份漂移；Plan 绑定的软件流水 subject 集合
没有覆盖最终事实中的全部 capable loop。这些缺陷影响 272 个“请求 × Route”单元，最终验证
均已安全拒绝，所以没有污染正确性，但确实缩小了候选空间。三类缺陷现已在共享候选编译器
中修复；最终源码下的 20 个受影响角色回放没有复现旧错误，两个仓库的
`combine_topk_swa_indices` 还从初始无优化胜者变为 7/7 请求均选择优化路线。修复、回放限制
和主 L 字节级对齐证据见
[`l-lite-priority-operators-43-report.zh-CN.md`](l-lite-priority-operators-43-report.zh-CN.md#9-2026-08-28缺陷修复受影响角色重测与主-l-对齐)。
这也验证了原先的边界判断：不能让主 L 的性能模型或残差去吸收候选身份错误。

第四，另有 528 个单元属于证明或组合能力尚未闭合，主要是动态 main/tail 证书缺失，以及
Bridge 构造外层循环后找不到唯一原生可流水主体。这些是待证明的能力扩展，不应在证明缺失时
直接改成可执行候选。还有 70 个单元暴露机会资格过宽：结构层认为可流水或可向量化，最终却
没有对应 artifact 或精确可打包操作组。正确修复位置是机会事实和 Route 准入，而不是降低
后置验证标准。

第五，定向批次进一步量化了 exhaustive autotune（穷举自动实测）的成本。`grouped_topk`
累计首次候选获取 4188.82 秒，`moe_sum` 为 1208.79 秒；`chunk_gla`、`chunk_gdn2`、
`add_rms_norm`、`addmm`、`rms_norm` 在 900 秒内持续生成候选机器码但未完成选择。这些结果强化
了主 L 的产品目标：共享同一候选能力，用因果预测、热点和收益下界避免对低价值 kernel 支付
完整穷举成本。
...
第一版按上述口径完成的真实算子对比见
[《Main L 与 L-lite 已知正收益算子对比实验》](main-l-vs-l-lite-positive-operators.zh-CN.md)。该实验严格区分“最低成功门已经通过”和“尚未恢复大部分 L-lite 收益机会”，并公开逐条机器可读证据。

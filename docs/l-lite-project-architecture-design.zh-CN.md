...
### 16.9 FlagGems（GPU 算子库）阶段性自然算子结果

FlagGems 与 FlagGems-vLLM（面向大模型推理的算子集合）的完整 V20（第 20
版测量协议）仍在连续运行。本节冻结的是 2026-08-26 07:05:03 UTC（协调世界时）的阶段性快照，而不是
最终全库结论。快照哈希为
`bd9055aabf54988a95a6f758651acda878632014ebbe827e6f5ddfb8fc76f318`。

截至快照，FlagGems 已得到 18 个环境合格、Original 与 L-lite 均完成的配对测试节点；共
执行 23 个测试实例，23 个通过、0 个跳过、0 个失败。当前尚未轮到 FlagGems-vLLM 的有效
配对结果，因此本节不提前报告其收益。

| 阶段性统计项 | 当前结果 | 如何理解 |
|---|---:|---|
| 已完成配对节点 | 18/141 | 只覆盖当前排在前面的 FlagGems 节点，不能外推到全库 |
| 内核请求 | 69 | 同一测试可能因 shape 或调用点产生多个请求 |
| 选择非 Original 候选 | 20 | 其中 16 个请求的点估计加速大于 1 |
| 69 个请求的几何平均加速 | 1.011× | 包含仍选择 Original、加速记为 1 的请求 |
| 20 个已优化请求的几何平均加速 | 1.039× | 只是已选择优化候选的条件统计 |
| 原生自动调优计时 | 369.72 秒 | 实际候选基准测量时间之和 |
| 首次获得候选结果的 acquisition（获取）时间 | 574.14 秒 | 包含候选生成、编译、验证和调优 |
| L-lite 相对匹配 Original 的额外进程墙钟时间 | 980.83 秒 | 当前批次 L-lite 为 1578.71 秒，Original 为 597.87 秒 |
| 提交候选 | 2258 | 自动调优前删除数量为 0 |
| 调优后类型化无效候选 | 1220 | 无效候选的尝试成本没有从对照组中隐藏 |
...
| 算子或内核 | 正收益覆盖 | 选择的主要变换 | factor | 点估计收益 | 该节点首次获取开销 | 当前判断 |
|---|---:|---|---|---:|---:|---|
| `broadcast_to_kernel` | 9/9 个请求 | 完全展开加重排 | Bridge 1，Route 16 | 几何平均 1.078×；范围 1.024×–1.149× | 218.29 秒 | 当前最完整、最一致的自然算子正收益 |
| `_kernel_H32_D128`，来自 BF16（脑浮点 16 位格式）分页 MQA（多查询注意力）logits（未归一化得分） | 1/6 个请求 | 完全展开加重排 | Bridge 2，Route 2 | 获胜请求 1.033× | 节点合计 33.39 秒 | 局部 shape 有收益，覆盖尚窄 |
| `_amp_foreach_non_finite_check_and_unscale_kernel` | 7/20 选择优化；其中 6 个大于 1 | 软件流水、完全展开加重排、完全展开加向量化 | Bridge 2/4，Route 2/4 | 6 个正收益请求几何平均 1.008×；范围 1.003×–1.027× | 286.66 秒 | 点收益很小，除非长期极热，否则当前获取成本难以摊销 |

`broadcast_to_kernel` 是当前最值得关注的结果。9 个请求全部选择同一类完全展开加重排
机制，且每个请求都快于 Original。原始 TTIR 中的实际循环、L-lite 可观测循环、原生软件
流水可处理循环、重排支持循环和向量化支持循环均为 1，说明该结果同时具备明确观察对象和
完整 Route 竞争，不是由缺少其他候选造成的单边获胜。

BF16 分页 MQA logits 只在 6 个请求中的 1 个获得 1.033×。它的原始 TTIR 循环计数为 0，
获胜 Route 作用于 Bridge 新构造的循环；因此这个结果验证了 Bridge 与重排 Route 可以联合
物化，但还没有形成跨 shape 的广泛收益。

AMP（自动混合精度）非有限值检查与缩放算子的 6 个正收益请求幅度很小，而且节点首次获取开销达到
286.66 秒。它说明三类 Route 都能在自然算子上进入竞争并偶尔获胜，但现阶段不能证明其
具有发布价值。主 L 若未来选择这些候选，必须结合长期热度证明运行期累计收益能够摊销
获取成本。

另有 `avg_pool2d` 的 L-lite 角色在候选已经实际提交后超时，现记为
POST_SUBMISSION_TIMEOUT_GAP（提交后超时缺口）。它没有合格的配对性能结果，因而没有被
塞入上面的平均值。完整 V20 结束后，本节将以全量 FlagGems 和 FlagGems-vLLM 结果替换；
在此之前，以上数字只作为已经获得的阶段性事实和后续回归基线。

#### 当前超时的原因分析

超时不能统一写成“测试太慢”。每个超时都必须区分至少四类原因：原测试本身耗时、L-lite
候选空间带来的编译与自动调优耗时、编译器停滞，以及外部环境不合格。截至本快照只有
`avg_pool2d` 一个超时角色，现有证据足以排除后两类，并把主要增量归因到 L-lite 的候选
枚举范围。

| 证据 | Original | L-lite | 结论 |
|---|---:|---:|---|
| 角色墙钟时间 | 82.55 秒，自然完成 | 900.30 秒，触及硬超时 | L-lite 至少增加 817.75 秒，墙钟时间至少为 Original 的 10.91 倍 |
| 已完成测试 | 前向、反向均完成 | 前向完成，反向未完成 | 不完整角色不能形成配对收益 |
| GPU 环境 | 合格 | 7332 次连续采样、0 次违规 | 不是其他进程、热降频或环境越权造成的超时 |
| 冷缓存编译源产物 | 178 份 | 492 份 | L-lite 路径产生了明显更大的编译工作量 |
| `randn_kernel` 编译源产物 | 3 份 | 212 份 | 输入随机数生成辅助内核也被纳入候选枚举 |
| `zeros_kernel` 编译源产物 | 1 份 | 143 份 | 清零辅助内核同样发生候选扩张 |
| 冷缓存总文件体积 | 32.42 MB（兆字节） | 184.27 MB | 后端编译产物体积约扩大到 5.68 倍 |

第一项 `test_avg_pool2d` 在 06:36:22 UTC 已通过。随后第二项
`test_avg_pool2d_backward`（二维平均池化反向测试）进入输入和梯度准备阶段；`randn_kernel` 的新编译产物从
06:36:22 后持续生成，最后一份写于 06:50:37，距离 06:50:41 的超时终止约 4 秒。因此该
进程一直在生成和编译候选，不符合“长时间没有进展”的死锁特征。超时后，进程组由预注册
的 SIGTERM（终止信号）完整回收，没有遗留新的编译子进程。

这项结果应当计入 autotune（自动调优）的获取耗时劣势，但不能夸大成“`avg_pool2d` 本体
有 355 个候选”。当前插件会观察测试进程中的所有 Triton 调用，基准测试为了产生输入而
调用的随机数和清零辅助内核也进入了 L-lite 笛卡尔积；212 和 143 是冷缓存中的编译源产物
数量，不是已经完整落账的候选请求数。因为第二项测试没有结束，逐请求报告尚未原子提交，
所以结果文件中的 `kernel_request_count`（内核请求数）仍为 0。这是“不完整请求不冒充完整结果”的正确
行为，同时也暴露了当前超时遥测只能事后从编译缓存重建工作量的不足。

从产品角度看，这证明穷举式 L-lite 在一次性、低复用和测试辅助 kernel 上可能付出很大的
首次获取成本；这正是主 L 使用热度门和预测选择、而不对所有候选逐一实测的价值之一。从
评测角度看，输入生成属于基准测试脚手架，不一定等于生产 workload（工作负载）中的目标
算子成本。因此报告保留 817.75 秒的实测增量下界，但把原因写成“全调用路径候选枚举”，
不把它错误归到 `avg_pool2d` Route 本身。V20 冻结运行期间不修改遥测协议；完整批次结束后，
下一版会为每个未完成请求保存只增不减的候选提交与编译进度，使后续每个超时都能直接按
目标 kernel、辅助 kernel、候选编译、原生计时和环境等待分项报告。

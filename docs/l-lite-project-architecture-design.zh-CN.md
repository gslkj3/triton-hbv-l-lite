...
> 修订日期：2026-08-26
>
> 上游基线：Triton `7c56a5e40f7fd928dfd5c72902d5def0097db73a`（3.6.0 开发基线）
>
...
1. Bridge 先判断能否把多个 Triton logical program 合并到一个 physical program，并把
   合并后的工作表达为显式循环；
2. Route 再对 Bridge 后的真实循环，或原 TTIR 中本来就存在的循环，选择软件流水、
   完全展开后重排、完全展开后向量化三种机制之一。

Bridge 与 Route 是两个独立的编译干预。它们分别证明合法性、分别保存 factor 和来源，
但在候选层通过笛卡尔积联合搜索。这个划分既保留因果归属，也没有放弃整体 kernel 选优。

本次源码审计得到三个结论。

第一，Bridge 的 program 独立性证明、重排的依赖图调度、一般操作的精确打包，已经是由
IR、SSA、内存效应和地址关系驱动的规则式实现。它们不读取 kernel 名、算子名、测试集名
或历史性能结果。

第二，评审提出的 case-by-case 风险确实存在，不能回避。当前 `exact-prefix` 路径仍匹配
“单 load、单整数加法归约”等窄结构；state-axis 路径虽然比较完整数据流图，但仍使用固定
operation capability 名单；总物化入口也以多条结构分支调用不同 adapter。它们是结构级
定制，不是 workload 名称定制，但仍没有完全达到本文定义的统一规则架构。
...
- 在收敛完成并通过正反例、变形测试和新 family holdout 之前，不能把窄 adapter 写成
  已闭环的通用能力。

本文后续按目标架构介绍系统，同时在每一节标明当前实现和差距。这样，架构图描述的是
要长期维持的责任边界，代码审计表描述的是当前 commit 实际达到的位置。
...
```mermaid
flowchart TB
    U["输入：Triton kernel、specialization、launch/grid、factor domain"]
    C["Loop Census<br/>发现原生循环与可构造的 Bridge subject"]
    G["Candidate Graph<br/>Original + Bridge factor × Route × Route factor × scope"]
    P["PlanBundle<br/>冻结 subject、factor、证书版本与候选 identity"]

    subgraph CC["共享 CandidateCompiler：L-lite 与主 L 的共同能力面"]
      D["Discover / Normalize<br/>恢复结构、绑定 program 轴、正规化控制流"]
      BR["Bridge Rule Engine<br/>提取事实并证明 program 独立性"]
      BM["Bridge Materializer<br/>构造 logical-program loop"]
      F["Post-Bridge Facts<br/>重新观察真实循环、依赖、effect、trip、nest"]
      RR["Route Rule Engine<br/>分别证明 pipeline / reorder / vectorize"]
      UN["Native LoopUnroll + lineage<br/>生成 main/tail 与 clone 来源"]
      RM["Generic Route Materializer<br/>原生流水 / 拓扑重排 / 等价图打包"]
      V["Postcondition Validator<br/>拒绝回退、身份漂移和不完整物化"]
      D --> BR --> BM --> F --> RR --> UN --> RM --> V
    end

    N["原生 Triton lowering<br/>TTGIR → LLVM IR → PTX/cubin"]
    A["Artifact + correctness qualification"]
    T["原生 autotune<br/>只对合法、真实物化且正确的候选计时"]
    W["winner、完整 acquisition 与逐候选结果"]
...
检查的 binding。非法候选保留在请求账本中，但不会被执行。
...
是否能形成 Bridge subject。它只建立 population，不决定 Bridge 或 Route 是否会带来收益。

**第三步，建立 Candidate Graph。** 控制层生成 Original，以及所有预注册的
`bridge_factor × route × route_factor × scope` 请求。某个 cell 后续可能被拒绝，但此时不能
因预计较慢而不生成它。

**第四步，序列化 PlanBundle。** 候选被绑定到唯一 subject、Bridge factor 4、Route
`vectorize`、Route factor 4、axis divisor、规则版本和 composition hash。此后任何 Pass 都
无权私自更换 factor 或作用对象。

**第五步，Discover 与正规化。** 编译器读取真实 TTIR，解析 program-id、地址表达式、
control flow 和 effect。若存在可等价正规化的提前 return 或 pure helper，在这一层将其转换
为 Bridge 规则能够理解的 canonical representation；无法证明等价则拒绝。

**第六步，Bridge 证明与物化。** Bridge Rule Engine 证明四个 logical program 的坐标可
恢复、尾部可守卫、写 footprint 不冲突且 effect 闭合。证书成立后，Bridge Materializer
构造 factor 4 的显式循环，并保存 logical lane 来源。

**第七步，重新提取 Facts。** 系统面对 Bridge 后真实 `scf.for`，重新计算 trip、carried
state、nest、operation graph、mask 和 effect。Bridge 前“没有循环”或“某操作在函数顶层”
等旧事实到此失效。

**第八步，Route 证明。** 向量化规则查找四个 lane 中有 lineage 对应的等价 operation graph，
逐节点验证 operation 语义、类型、属性、operand、mask、地址和 effect。Bridge 证书只说明
program 可以合并，不能替代这一步。

**第九步，展开与物化。** 原生 LoopUnroll 按 factor 4 产生 clones 并保留来源。通用
vectorization kernel 依据等价组构造宽值和宽 operation。若只找到两路等价，不能擅自改成
factor 2；当前 factor 4 候选应失败，factor 2 由另一个候选 cell 表达。

**第十步，后置验证。** Validator 检查目标 subject 已被正确消费、packing artifact 确实
出现、Bridge 和 Route lineage 仍对应 PlanBundle，且临时 role 属性没有泄漏。任何不一致都
令候选编译失败。

**第十一步，原生 lowering 与资格检查。** 合格 TTIR 继续经过 TritonGPU、LLVM 和目标后端。
系统保存 artifact identity，并与 Original 和独立 oracle 做数值比较。只有物化和数值均
合格的 binding 才进入计时。
...
嵌套 region 和 sibling state 不应各自拥有一套从发现到物化的私有优化器。它们只能在必要
...
其中 `bridge_factor=1` 是 Bridge identity。它表示不构造 program-coarsening 循环，但不
...
- `scf.for` 的上下界、步长、静态或动态 trip、嵌套关系和 carried value；
- SSA def-use、dominance、region 结构和 operation effect；
- pointer root、仿射偏移、mask、alignment、cache/eviction 属性和读写 footprint；
- program-id 轴、grid extent 和多轴 mixed-radix 映射；
- dtype、tensor shape、显式资源或代码体积预算；
- PlanBundle 中预注册的 subject、route、factor 和结构 scope。

禁止读取的内容包括：

- Python 函数名、kernel 名、源码路径和算子库身份；
- “GEMM”“Sinkhorn”“Hadamard”等 workload 标签；
- benchmark、测试集或 PR 编号；
- 某个 shape 过去是否获胜；
- 计时结果、温度、功率、时钟、GPU utilization；
- 为某份开发数据手写的操作数量组合。

最后一项尤其重要。删除 kernel 名不等于去掉定制。如果规则要求“恰好四个 div，后接一个
add，再接四个 div”，它仍在用操作配方识别一个 workload，只是把名字换成了结构签名。
...
| 正确性 | 与 Original 和独立 oracle 是否一致 | 标记 incorrect/ambiguous，不计时 |
...
| 顺序 | 阶段 | 责任 | 不应承担的责任 |
|---|---|---|---|
| 1 | Discover | 发现原生循环和可构造 Bridge 的 program 事实 | 不改写、不选 winner |
| 2 | Bridge Program Coarsening | 按已证明证书构造 logical-program loop | 不判断 Route 是否有收益 |
| 3 | Facts | 对 Bridge 后真实 IR 重新提取 loop/effect/nest 能力 | 不复用 Bridge 前旧假设 |
| 4 | Decision | 将 PlanBundle 与事实、证书和唯一 subject 对齐 | 不静默改 factor 或 route |
| 5 | 原生 LoopUnroll | 按 factor 展开并保存 main/tail、clone lineage | 不做 L-lite 性能选择 |
| 6 | Materialize | 调用流水、重排或向量化通用内核 | 不吞掉失败后返回 Original |
| 7 | Validate | 检查计划、物化结果和来源一一对应 | 不以“Pass 退出码为 0”代替检查 |

新增 Pass 的注册位于
[`Passes.td`](../include/triton/Dialect/Triton/Transforms/Passes.td#L93-L125)，实现集中在
[`HBVLoop.cpp`](../lib/Dialect/Triton/Transforms/HBVLoop.cpp)，Python wrapper 位于
[`passes.cc`](../python/src/passes.cc#L39-L59)。

原生 LoopUnroll 的扩展位于
[`LoopUnroll.cpp`](../lib/Dialect/Triton/Transforms/LoopUnroll.cpp)。扩展只增加 source trip、
main/tail partition 和 clone operation group 来源，不另写一套展开器。这使 L-lite 能继续
复用 Triton/MLIR 的展开语义，同时让后续调度和验证知道每个 clone 来自哪次迭代、哪个
source operation。

## 5. 统一规则模型

### 5.1 Canonical Subject

所有规则首先面对统一对象，而不是 benchmark 子域。一个 canonical loop subject 至少包含：

```text
SubjectRef
LoopDomain      = {lower, upper, step, static/dynamic trip}
StructuralScope = {loop, nest path, bridge origin}
StateBoundary   = {iter args, yields, recurrence edges}
EffectSummary   = {reads, writes, atomics, volatile, unknown effects}
AddressSummary  = {pointer roots, affine/symbolic footprints, masks}
ControlSummary  = {guards, early exits, nested regions, calls}
Lineage         = {source op, logical lane, main/tail partition}
```
...
显式循环、边界 guard、nest 和 operation graph 与 Bridge 前不同。使用 Bridge 前的 trip 或
effect 直接决定 Bridge 后 Route，是典型的阶段穿透错误。

事实应携带 producer 和版本。例如：

```text
fact: program_partition_disjoint
producer: TritonLoopBridgeDiscover
schema: bridge_pid_partitioned_disjoint_v1
...
| operation 语义能力 | 依据 MLIR interface/trait 或明确代数性质判断 | 允许，需有可审计语义来源 |
| workload matcher | 读取函数名、算子名、仓库、PR 或 benchmark | 禁止 |
| 配方 matcher | 要求固定操作数量和固定组合，只为覆盖已知样本 | 禁止 |
| 性能回忆规则 | 因历史样本快而准入 | 禁止进入合法性层 |

例如，`arith.addf` 和 `arith.mulf` 的物理实现不同，Pass 可以依据 operation 的 trait、类型和
属性分别调用合法的 clone/pack 逻辑；这属于 operation 语义分派。若代码要求“四个除法、
一个求和、再四个除法”才识别 state-axis，则属于配方 matcher。

## 6. Bridge：规则式 program coarsening
...
若最后一组不足四个，Bridge 为每个 logical lane 建立边界 predicate。它不能越过原 grid，
...
predicate。函数级提前 return 必须能正规化为当前 lane 的局部 guard。

**B3：外部可见写互不冲突。** 对每个 store，证明相邻 logical program 的写 footprint 不
重叠；同一 pointer root 上存在读写时，还要排除跨 program alias。

**B4：effect 闭合。** atomic、volatile、未知 memory effect、不可解析 call 或跨 program
...
| 入口有提前 return | 将函数退出正规化为 continuation guard | B2、B4 |
| 主体调用 pure helper | 在 call graph 闭合后内联或透明分析 | B4 |
| store 藏在 `scf.if` | effect walker 递归进入 region | B3、B4 |
...
`if (looksLikeKnownKernel)` 分支。

## 7. 完全展开后重排：依赖图上的稳定拓扑调度

### 7.1 目的

完全展开把多次迭代复制到同一 block。原生展开通常保持 iteration-major 顺序：先放完第
0 次迭代的操作，再放第 1 次。重排 Route 在不违反依赖的前提下，把来自同一个 source
operation 的 clones 尽量排在一起，形成 phase-major 顺序。

重排不是“把 load 放前面、compute 放中间、store 放后面”。该描述只适用于某些循环，不能
作为准入条件。通用定义是：
...
**R1：展开边界闭合。** loop 的 yield 数量必须与 init args 对应，main/tail 语义可由原生
LoopUnroll 正确表达。

**R2：SSA 顺序不可破坏。** 若 operation B 的 operand 来自 A，则调度图必须包含 `A → B`。

**R3：可观察 effect 顺序不可猜。** effectful 或不可 speculatable 的 operation 按源顺序
加入 barrier chain。只有额外证明两个 memory effect 独立，才允许删除相应顺序边。

**R4：tail 保持原顺序。** 非整除 factor 的 tail 不能参与未经证明的跨边界重排。

**R5：排序确定。** 多个合法拓扑序中，用 source group rank 和原 block 顺序做稳定 tie-break。
同一 IR、同一 Plan 必须生成相同结果。

**R6：嵌套范围显式。** 一次候选只使用一个 route identity。若作用于 loop nest，scope 必须
明确给出被展开的组合 extent；不能在 inner 偷换成另一条 Route。
...
保守 barrier chain；调度优先级来自 clone lineage，不来自 opcode 配方。
...
operation-neutral 稳定拓扑调度已经是可复用核心。源码中仍保留早期针对 load/compute/
reduction 形态的窄证书函数。后续必须将 active registry 限定到通用依赖证书，历史证据读取
与生产准入物理分离；不得让旧 matcher 因调用顺序变化重新成为 authority。
...
标量或窄 tensor 操作组织成更宽值，在宽值上执行等价操作，再将结果交给原 consumer。
...
一致。共同 invariant operand 可以 broadcast；lane-local operand 必须递归建立对应组。

**V3：operation 可打包。** regionless、纯、可 speculatable 的 elementwise operation 可依据
MLIR trait 进入通用 pack；有 region 或特殊语义的 operation 需要明确的语义接口，不能靠
名称相似放行。

**V4：内存语义精确。** load/store 还要比较 pointer 映射、mask、other、boundary、cache、
eviction 和地址冲突。store lane 必须无冲突，且不能破坏有意义的覆盖顺序。

**V5：控制与 recurrence 闭合。** 被打包节点不能携带未建模的跨 lane recurrence。cross-state
reduction 要作为图中的显式边处理，不能假设 sibling 永远独立。

**V6：动态有效集合可表达。** 动态 lane 只有在存在可证明上界、有效集合可由 predicate
表达、无效 lane 有语义正确的 neutral value 时才能放入静态容器。
...
使用不同的 operation verifier 是必要的语义分派。为某个算子组合专门写“先打包四个 load，
...
```text
lane-local node：每个 sibling 各自存在、彼此同构的节点
uniform node：所有 sibling 共享的 invariant
cross-lane node：显式合并 sibling 的 reduction/consumer
boundary node：暂时无法继续向上打包、但可以安全形成容器边界的值
```
...
1. 有效集合确实是连续前缀；
2. `W` 是可证明上界；
3. 被替换操作具有明确的 lane-wise 或 reduction 语义；
4. neutral value 与 combiner 的代数性质已证明。
...
- `HBVLoopMaterializePass` 仍为 exact-prefix、state-axis、Bridge logical、runtime guarded 等
  路径保留显式分支，尚未形成统一的 adapter registry。

因此，本次评审后将 exact-prefix 和 state-axis 标记为 `PARTIAL`，不能以“已经支持某个样例”
表述为通用能力。它们重新进入 active authority 的条件见第 12 节。

## 9. 软件流水：复用原生 Triton 机制

软件流水 Route 不由 L-lite 重写后端 pipeline scheduler。L-lite 的责任是：

1. 定位准确的 live-loop subject；
2. 证明该 subject 具备原生 pipeline 所需的加载服务和跨迭代依赖条件；
3. 写入并保存 stage 请求；
4. 由原生 TritonGPU pipeline 建立实际 schedule；
5. 在后续 artifact 层确认请求没有被静默消除。

Bridge 能构造 `scf.for`，不代表该循环必然可流水化。一个只有 store 的 Bridge loop 仍可能
适合重排，但没有可提前搬运的 load service，软件流水应 route-local 拒绝。这个拒绝不能
污染重排和向量化的合法性。
...
- 用 location、source path 或 hash 代替 workload 名；
...
| 实现位置 | 当前性质 | 判定 |
|---|---|---|
| `certifyBridgeProgramIndependence` | pointer root、footprint、effect、alias 规则 | `RULE-BASED` |
...
| `matchExactPrefixReduction` | 单 pointer/load/整数 add 的窄形态 | `STRUCTURAL CASE` |
| `isStateAxisElementwiseCapability` | 固定 operation 名单 | `LIMITED CAPABILITY LIST` |
| `matchStateAxisNormalization` | 数据流图驱动，但入口仍围绕共享 denominator 形态 | `PARTIAL` |
| `HBVLoopMaterializePass` 总分派 | 多条 adapter 分支，无正式统一 registry | `ARCHITECTURE DEBT` |
| 历史 load/compute/reduction 证书 | 早期窄结构仍留在同一源文件 | `AUTHORITY RISK` |
...
丢失。它不能只相信 materializer 返回 `success()`。

### 11.2 exact-prefix 整改

不再以“一次 load + AddI”定义 exact-prefix。新的正规化器只负责识别 `[0,N)` 有效域和静态
上界；operation/reduction 的合法性由通用 operation capability 与 monoid certificate 决定。

如果 combiner 没有可证明的 identity、结合律要求与数值合同，则拒绝。不能为了覆盖已知
PR 再添加一个 opcode 组合。原窄 matcher 可保留为历史证据 reader，但不得注册为生产规则。
...
识别入口从“寻找某个共享 denominator”改为“从多个候选 sibling outputs 反向建立最大同构
...
```text
adapter_id
input structural predicate
normalization equivalence proof
produced canonical subject schema
compatible routes
post-normalization invariants
positive tests
earliest-layer negative tests
```
...
当前 `HBVLoop.cpp` 超过一万行，事实、历史 matcher、Plan parser、物化器和 validator 混在
同一翻译单元。建议拆为：
...
拆分的目的不是整理文件，而是让依赖方向可检查：historical reader 不能被 active registry
调用；Facts 不读取性能；Materialize 不修改 Plan；Validate 不补做变换。

## 12. 规则式能力的验收标准

一条能力只有同时满足以下条件才能标记 `PROVED`。
...
- 一个 metamorphic 变形测试，例如重命名、插入无关纯操作、改变 sibling 数量或等价重排后
  结论不应变化；
...
Original loop
Bridge-created loop
static divisible trip
static main/tail
dynamic guarded trip（若声明支持）
nested scope（若声明支持）
```

每个组合 cell 都有唯一 disposition。没有被执行的非法候选仍保留 identity 和拒绝原因，
但不交给 GPU。
...
| 一键 CandidateCompiler | `MISSING` | 公开 binding 工厂与统一 pipeline |
...
### 13.1 factor ontology

[`factor_ontology.py`](../python/triton/l_lite/factor_ontology.py) 定义 Bridge factor、Route factor、
subject、subtype 和 nested scope。factor 的含义按机制区分：

| 字段 | 含义 |
|---|---|
| Bridge factor | 一个 physical program 合并的 logical program 数 |
| pipeline factor | 原生软件流水 stage 请求 |
| reorder factor | 参与完整展开和依赖图重排的组合 extent |
| vector factor | 一次等价图打包的 lane 数 |

嵌套循环仍只有一条 Route 和一个 Route factor。scope 可以表示 outer、inner 或组合 extent；
...
### 13.3 composition graph

[`composition.py`](../python/triton/l_lite/composition.py) 枚举 Original 和完整请求网格，保存
稳定 candidate identity。请求失败不等于从图中删除；它应有类型化 rejection，便于计算完整
acquisition，并防止 L-lite 与主 L 使用不同候选全集。

### 13.4 PlanBundle

PlanBundle 是 Python 与 C++ 的闭合协议，至少包含：

- schema/adapter 版本；
- candidate、subject 和 Provider ref；
- Bridge factor 与多轴 divisor；
- Route、subtype、Route factor 和 scope；
- main/tail、动态边界和资源预算；
- composition hash。

未知字段、缺字段、重复 subject 或版本不匹配必须 fail-closed。后续应从同一 schema 生成
Python serializer 和 C++ parser，减少双实现漂移。

### 13.5 autotune 边界

[`autotune.py`](../python/triton/l_lite/autotune.py) 只接收已经编译并通过验证的 bindings。它
复用原生 Triton Autotuner 的计时、winner 选择和 key cache。L-lite 不替 autotune 预先删除
“可能很慢”的合法候选；但语义非法、物化失败或数值错误的候选本来就不是可执行候选，
不能为了表面公平交给 GPU。

## 14. 相对原生 Triton 的改动

原生 Triton 已提供 SCF/TTIR 基础设施、LoopUnroll、TritonGPU 软件流水、后端 codegen 和
Autotuner。L-lite 在这些能力上增加候选身份、证明规则、Bridge、两条 Route 物化和验证。

| 文件 | 改动 | 架构作用 |
|---|---|---|
| [`Passes.td`](../include/triton/Dialect/Triton/Transforms/Passes.td) | 注册六个 Pass | 让分析/变换进入正式 PassManager |
...
| [`composition.py`](../python/triton/l_lite/composition.py) | 候选图和 attestation | 保存完整请求账本 |
| [`autotune.py`](../python/triton/l_lite/autotune.py) | 原生 autotune facade | 只负责实测选择 |
...
候选 binding 的唯一公开入口。建议提供显式 opt-in API：
...
必须包含 store-only、compute-only 和混合 effect，防止“必须有 load”之类配方重新出现。
向量化测试要改变 operation 名、属性、mask、sibling 数量和 graph shape，确认规则依据是
语义而非样本外观。
...
性能数据必须记录 GPU UUID、驱动、时钟策略、外部 PID、预热、重复次数、统计量和测量
...
```mermaid
flowchart TB
    C["共享 CandidateCompiler<br/>同一候选域、同一 artifact、同一 correctness"]
    LL["L-lite：全部可执行候选真实计时"]
    ML["主 L：因果域模型预测 + 安全发布门"]
    AW["Autotune winner"]
    HW["HBV release / Original"]
    C --> LL --> AW
    C --> ML --> HW
```
...
后端寄存器分配、spill 和调度选择不完全由上游 TTIR 可见。主 L 不要求完全白盒复制后端，
...
| 目标 | 找到真实 exhaustive winner | 用更低 acquisition 获得安全净收益 |

结果至少报告最终候选性能、总 acquisition、break-even reuse count、false adoption 和保守
...
### P1：完整 CandidateCompiler

1. 提供唯一 `candidate → PlanBundle → binding` 入口；
2. V2 候选图接入 native autotune；
3. 完整 pipeline、artifact 和 correctness harness；
4. 逐候选 acquisition 与 typed disposition。
...
- [ ] artifact、correctness 和 timing identity 一一对应；
- [ ] 高精度性能结论有合格环境证据；
- [ ] 未完成项使用 `PARTIAL/MISSING`，不以局部成功扩大结论。

建议状态只使用四种：`PROVED`、`PARTIAL`、`CONTRADICTED`、`MISSING`。没有反例证据不等于
`PROVED`；只有实现、没有同范围验收时应标 `PARTIAL`。

## 22. 代码阅读顺序

1. [`factor_ontology.py`](../python/triton/l_lite/factor_ontology.py)：factor、subject、subtype；
2. [`contract.py`](../python/triton/l_lite/contract.py)：Bridge 后对象与 Route 合同；
3. [`composition.py`](../python/triton/l_lite/composition.py)：候选图和 identity；
4. [`Passes.td`](../include/triton/Dialect/Triton/Transforms/Passes.td#L93-L125)：六个 Pass 注册；
5. [`HBVLoop.cpp` 的 Bridge 证明](../lib/Dialect/Triton/Transforms/HBVLoop.cpp#L2152)；
...
10. [`LoopUnroll.cpp`](../lib/Dialect/Triton/Transforms/LoopUnroll.cpp)；
11. [`materializeOperationNeutralPhase`](../lib/Dialect/Triton/Transforms/HBVLoop.cpp#L8124)；
12. [`materializeExactOperationVectorization`](../lib/Dialect/Triton/Transforms/HBVLoop.cpp#L9413)；
...
15. [`autotune.py`](../python/triton/l_lite/autotune.py)：原生 autotune 边界。

## 23. 术语

| 术语 | 定义 |
|---|---|
| logical program | Triton 以 program-id 区分的逻辑并行工作单元 |
| physical program | Bridge 后实际 launch 的 program，一个可承载多个 logical program |
| Bridge | 将 program coarsening 表达为显式循环的前置变换 |
| Route | 软件流水、完全展开后重排、完全展开后向量化之一 |
| subject | 一次分析或变换准确作用的 IR 对象 |
| FactSet | 从当前 IR 直接提取、带 producer 的事实集合 |
| certificate | 某条证明规则对指定 subject 成立的类型化证据 |
| adapter | 将不同 IR 表示正规化为 canonical subject 的等价转换 |
| lineage | operation、loop 和候选从来源到 artifact 的身份链 |
| materialization | 把计划真实改写为目标 IR 结构 |
| postcondition | 物化后必须重新观察到的事实 |
| silent fallback | 请求失败后退回 Original，却仍冒充目标候选 |
| acquisition | census、编译、验证、正确性和计时的全部前期开销 |
| statistical proxy | 在不可完全观测的后端边界上使用的因果域专属统计模型 |

## 24. 参考设计资料

本文的 Pass 分层、分析与变换责任边界参考 MLIR 官方
[Pass Infrastructure](https://mlir.llvm.org/docs/PassManagement/)；canonical loop 语义参考
[SCF Dialect](https://mlir.llvm.org/docs/Dialects/SCFDialect/)；候选计划与最终 IR 生成分离的
设计思路参考 LLVM [Vectorization Plan](https://llvm.org/docs/VectorizationPlan.html)。
...
L-lite 的正确定位是一个规则驱动的循环候选编译与 exhaustive 对照系统：Bridge 负责安全
